...
class NetSim {
    std::string OUTPUT_DIR = "output/experiment2/";  // OUTPUT_DIR:=(ログファイルの出力ディレクトリ).
    ...
    bool m_enableEnemy;                  // m_enableEnemy:=(外敵を有効にする).
...
NetSim::NetSim(int argc, char *argv[]) {
    ...
    m_tracing = false;
    m_verbose = false;
    m_prefix = "experiment2";

    m_vn = 26;  // tower and boids.
    m_dist = 40.0;
    m_height = 30.0;
    m_enableEnemy = false;
    m_simStop = 100.0;
