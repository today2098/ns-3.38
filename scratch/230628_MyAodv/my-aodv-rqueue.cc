...
NS_LOG_COMPONENT_DEFINE("MyAodvRequestQueue");
...
void RequestQueue::DropPacketWithDst(Ipv4Address dst) {
...
bool RequestQueue::Dequeue(Ipv4Address dst, QueueEntry& entry) {
    Purge();
    for(std::vector<QueueEntry>::iterator i = m_queue.begin(); i != m_queue.end(); ++i) {
        if(i->GetIpv4Header().GetDestination() == dst) {
            entry = *i;
...
bool RequestQueue::Find(Ipv4Address dst) {
    for(std::vector<QueueEntry>::const_iterator i = m_queue.begin(); i != m_queue.end(); ++i) {
        if(i->GetIpv4Header().GetDestination() == dst) {
            return true;
        }
...
void RequestQueue::Drop(QueueEntry en, std::string reason) {
