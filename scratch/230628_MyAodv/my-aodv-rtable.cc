...
NS_LOG_COMPONENT_DEFINE("MyAodvRoutingTable");
...
void RoutingTableEntry::GetPrecursors(std::vector<Ipv4Address>& prec) const {
    NS_LOG_FUNCTION(this);
...
void RoutingTableEntry::Invalidate(Time badLinkLifetime) {
...
void RoutingTableEntry::Print(Ptr<OutputStreamWrapper> stream, Time::Unit unit /* = Time::S */) const {
...
bool RoutingTable::LookupValidRoute(Ipv4Address id, RoutingTableEntry& rt) {
    NS_LOG_FUNCTION(this << id);
...
bool RoutingTable::SetEntryState(Ipv4Address id, RouteFlags state) {
    NS_LOG_FUNCTION(this);
...
void RoutingTable::InvalidateRoutesWithDst(const std::map<Ipv4Address, uint32_t>& unreachable) {
    NS_LOG_FUNCTION(this);
    Purge();
...
void RoutingTable::Purge(std::map<Ipv4Address, RoutingTableEntry>& table) const {
    NS_LOG_FUNCTION(this);
    if(table.empty()) {
        return;
    }
...
bool RoutingTable::MarkLinkAsUnidirectional(Ipv4Address neighbor, Time blacklistTimeout) {
...
void RoutingTable::Print(Ptr<OutputStreamWrapper> stream, Time::Unit unit /* = Time::S */) const {
